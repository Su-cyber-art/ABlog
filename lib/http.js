...
function parseMultipart(raw, contentType) {
  ...
  while (pos < raw.length) {
    if (raw.subarray(pos, pos + 2).toString('ascii') === '--') break;
    if (raw.subarray(pos, pos + 2).toString('ascii') !== '\r\n') break;
    pos += 2;

    const headerEnd = raw.indexOf(headerSeparator, pos);
    if (headerEnd < 0 || headerEnd - pos > 16 * 1024) break;
    ...
    if (!nameMatch || !/^[A-Za-z0-9_-]{1,80}$/.test(nameMatch[1])) break;
    ...
    // separator 自带前导 CRLF；越过完整 boundary 后，下一轮再判断 CRLF(普通分段)或 --(结束分段)。
  ...
  async handle(req, res) {
    ...
    if (req.method === 'POST') {
      ...
      if (type.includes('application/x-www-form-urlencoded')) {
        ...
        const parsed = parseMultipart(raw, type);
