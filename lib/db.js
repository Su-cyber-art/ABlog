...
function siteSettings() {
  ...
  const portraitUrl = /^portrait\.(?:jpg|png|webp)$/.test(portraitFile)
    ...
    : '';
  const faviconCustom = getSetting('favicon_file', '') === 'favicon.png';
  const faviconUrl = faviconCustom
    ? '/uploads/favicon.png?v=' + encodeURIComponent(getSetting('favicon_updated', ''))
    : '/favicon.svg';
  return {
    ...
    locale: normalizeLocale(getSetting('locale', 'zh-CN')),
    perPage: Math.max(1, parseInt(getSetting('perPage', '5'), 10) || 5),
    portraitUrl,
    faviconUrl,
    faviconType: faviconCustom ? 'image/png' : 'image/svg+xml',
    faviconCustom
  };
}
