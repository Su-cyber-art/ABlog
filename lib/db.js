...
try {
  ...
  console.error('[默·博客] 需要 Node.js 22.5 及以上版本(内置 SQLite)。当前版本:' + process.version);
...
/** 恢复示例数据(保留登录密码与会话密钥) */
const seedAll = () => tx(() => {
  db.exec('DELETE FROM posts; DELETE FROM comments; DELETE FROM cats; DELETE FROM tags;');
...
if (!getSetting('admin_pass')) {
  const initial = process.env.ADMIN_PASSWORD || 'mo-admin';
  setSetting('admin_pass', hashPassword(initial));
  if (!process.env.ADMIN_PASSWORD) {
    console.log('[默·博客] 后台初始密码：mo-admin（请登录后在「站点设置」中修改）');
  }
}
...
const q = {
  ...
  searchPosts: db.prepare(
    "SELECT * FROM posts WHERE status='published' AND (title LIKE ?1 ESCAPE '\\' OR content LIKE ?1 ESCAPE '\\' OR tags LIKE ?1 ESCAPE '\\') ORDER BY date DESC, id DESC LIMIT 50"
  ),
...
module.exports = { db, q, tx, getSetting, setSetting, siteSettings, seedAll, parseTags };
