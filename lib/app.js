...
function buildApp() {
  ...
  app.usePre((req, res) => {
    ...
    let actual = '';
    let expected = SITE_URL;
    try { actual = new URL(origin).origin; } catch (e) { /* origin 为 null 或非法 */ }
    if (!expected) {
      try { expected = new URL('http://' + String(req.headers.host || '')).origin; } catch (e) { expected = ''; }
    }
    if (!actual || actual !== expected) res.text('跨站请求被拒绝', 403);
  });
  ...
  app.setBodyLimit(req => {
    ...
    if (req.isAdmin && req.pathname === adminUrl('/editor/save')) return 600 * 1024;
    return 64 * 1024;
  });
